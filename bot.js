...
async function runBot() {
    ...
    // ==========================================
    // NODE.JS DATA SAVER (Permanent Storage)
    // ==========================================
    ...
    await page.evaluate(() => {
        ...
        ui.innerHTML = `
            <div style="font-weight: bold; font-size: 13px; color: #ffeb3b; margin-bottom: 5px;">⚡ Infinite Bot v25.0</div>
            <div id="bot-action" style="font-size: 11px; margin-bottom: 8px; color: white;">Initializing...</div>
            <button id="bot-toggle" style="width:100%; background:#f44336; color:white; border:none; padding:5px; cursor:pointer;">PAUSE</button>
        `;
        ...
        document.getElementById('bot-toggle').addEventListener('click', (e) => {
            isRunning = !isRunning;
            e.target.style.background = isRunning ? '#f44336' : '#4CAF50';
            e.target.textContent = isRunning ? 'PAUSE' : 'RESUME';
            document.getElementById('bot-action').textContent = isRunning ? 'Running...' : 'Paused';
        });
        ...
        // 4. HELPER FUNCTIONS & DOM CLICKER
        ...
        function hasPlayerRating95OrAbove() {
            const walker = createSafeTreeWalker(document.body);
            let node;
            const screenMiddle = window.innerWidth / 2;
            while ((node = walker.nextNode())) {
                ...
                if (!isNaN(num) && num >= 95 && num <= 99 && text === num.toString()) {
                    const parent = node.parentElement;
                    if (window.getComputedStyle(parent).display !== 'none' && isElementOnTop(parent)) {
                        const rect = parent.getBoundingClientRect();
